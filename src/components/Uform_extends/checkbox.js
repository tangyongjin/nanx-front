    ...
    render() {
        ...
        return (
            <div style={{ marginBottom: '10px' }}>
                {this.state.checkboxList.length > 0 ? <Checkbox.Group value={selectedvalue} onChange={this.onChange}>
                    {
                        this.state.checkboxList.map((item,index) => {
                            return <Checkbox style={{ display:'block',margin:'0px' }} key={index} value={item}>{item}</Checkbox>
                        })
                    }
                </Checkbox.Group> : null}
